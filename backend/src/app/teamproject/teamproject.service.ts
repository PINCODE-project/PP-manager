...
export class TeamprojectService {
    ...
    async parseProject(parseProjectDto: ParseProjectDto) {
        ...
        await fetch(
            "https://teamproject.urfu.ru/api/v2/workspaces/" + parseProjectDto.id + "/documents/results",
            requestOptions,
        )
            .then((response) => response.json())
            ...
            for (const student of thematicGroup.students) {
                if (student) {
                    if (!(await this.studentService.isCreate(student.userId))) {
                        ...
                        await this.studentService.update(student.userId, {
                            id: student.userId,
                            fullname: student.fullname,
                            phone: student.contacts.phone,
                            email: student.contacts.email,
        ...
        for (let studentResult of studentsResults) {
            if (
                await this.studentProjectResultService.isCreateByIds(studentResult.student.userId, parseProjectDto.id)
            ) {
                await this.studentProjectResultService.update(studentResult.student.userId, parseProjectDto.id, {
                ...
                this.logger.log(
                    "\tUpdate student project result: (student:" +
                ...
                this.logger.log(
                    "\tCreate student project result: (student:" +
                        studentResult.student.userId +
                        ", project: " +
                        parseProjectDto.id +
                        ")",
    ...
    async createReport(createReportDto: CreateReportDto) {
        ...
        projects.forEach((project, index) => {
            ...
            JSON.parse(project.team).thematicGroups.forEach((group, groupIndex) => {
                group.students.forEach((student, studentIndex) => {
                    if (!students.find((s) => s.key === student.userId)) students.push(student.fullname);
                });
            });
            ...
            projectsSheet["G" + (index + 2)] = {
                t: "s",
                v: project.passport?.request?.customer_user?.customer_company?.name,
            };
                ...
                group.students.forEach((student, studentIndex) => {
                    if (!students.find((s) => s.key === student.userId))
                        students.push({
                            ...
                            finalScore:
                                JSON.parse(project.results)?.thematicGroups?.[groupIndex]?.students[studentIndex]
                                    ?.finalScore || "Нет оценки",
                            retakedScore:
                                JSON.parse(project.results)?.thematicGroups?.[groupIndex]?.students[studentIndex]
                                    ?.retakedScore || "Не пересдавал",
                        });
                });
            });
        });
        ...
        students.forEach((student, index) => {
            ...
            usersSheet["F" + (index + 2)] = {
                t: student.finalScore === "Нет оценки" ? "s" : "n",
                v: student.finalScore,
            };
