...
export class AnalyticService {
    ...
    async getMainAnalytics(getMainAnalyticsDto: GetMainAnalyticsDto) {
        ...
        for(let project of projects) {
            if(project.isHaveReport)
                hasReportCount++;
            if(project.isHavePresentation)
                hasPresentationCount++;
            if(project.comissionScore !== null)
                hasCommissionScoreCount++;

            if(!marks.hasOwnProperty(project.comissionScore))
                marks[project.comissionScore] = 1;
            else
                marks[project.comissionScore] += 1;
        }
